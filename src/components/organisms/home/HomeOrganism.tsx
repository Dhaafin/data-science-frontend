...
import { ArrowUp, CircleNotch, MapPin, MusicNote, ChartBar, Globe, Warning, Info, MagnifyingGlass, ArrowsDownUp, Question, CaretDown, CaretUp } from "@phosphor-icons/react";
...
export default function HomeOrganism() {
  ...
  const [isRq2Transitioning, setIsRq2Transitioning] = useState(false);
  ...
  // RQ2: City Genre Breakdown (City Focus)
  const cityGenreFocusStats = useMemo(() => {
    if (artists.length === 0) return [];

    // Group artists by normalized city
    const cityMap = new Map<string, { name: string; count: number; genreMap: Map<string, number> }>();

    artists.forEach((art) => {
      const city = normalizeCity(art.originCity);
      if (!city || city.toLowerCase() === "unknown") return;

      const current = cityMap.get(city) || { name: city, count: 0, genreMap: new Map<string, number>() };
      current.count += 1;
      ...
      current.genreMap.set(formattedGenre, (current.genreMap.get(formattedGenre) || 0) + 1);
      cityMap.set(city, current);
    });
    ...
    return Array.from(cityMap.values())
      ...
      .map((c) => {
        ...
        const genres = Array.from(c.genreMap.entries())
          .map(([name, count]) => {
            const spotlightDef = SPOTLIGHT_GENRES.find(
              (g) => g.name.toLowerCase() === name.toLowerCase() || g.displayName.toLowerCase() === name.toLowerCase()
            );
            const color = spotlightDef ? spotlightDef.color : "#94a3b8";
            return {
              name,
              count,
        ...
        <section id="research-analysis" className="scroll-mt-20">
          {isDataLoading ? (
            ...
            <div className="flex flex-col gap-8">
                ...
                <GlassCard className="p-6 flex flex-col gap-6">
                      ...
                      <div className="flex p-0.5 bg-black/40 rounded-lg border border-white/5">
                        {[
                          ...
                          <button
                            key={gran.value}
                            onClick={() => {
                              if (rq1Granularity === gran.value) return;
                              setIsRq1Transitioning(true);
                              setRq1Granularity(gran.value as any);
                              setTimeout(() => setIsRq1Transitioning(false), 300);
                            }}
                            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                  ...
                  <AnimatePresence mode="wait">
                    {isRq1Transitioning ? (
                      ...
                      >
                        {/* Skeleton layout based on target mode */}
                        {rq1Granularity === "pulau" && (
                          <div className="flex flex-col gap-6 animate-pulse">
                            ...
                            <div className="flex flex-col gap-3">
                              ...
                              <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20 p-3 flex flex-col gap-3">
                                <div className="grid grid-cols-4 p-3 bg-white/5 border-b border-white/5">
                                  <Skeleton className="h-3 w-1/2 rounded bg-white/10" />
                                  <Skeleton className="h-3 w-12 rounded bg-white/10 justify-self-center" />
                                  <Skeleton className="h-3 w-16 rounded bg-white/10 justify-self-end" />
                                  <Skeleton className="h-3 w-10 rounded bg-white/10 justify-self-end" />
                                </div>
                                {[...Array(3)].map((_, i) => (
                                  <div key={i} className="grid grid-cols-4 p-3 border-b border-white/5 last:border-b-0">
                                    <Skeleton className="h-4 w-1/2 rounded bg-white/10" />
                                    <Skeleton className="h-4 w-10 rounded bg-white/10 justify-self-center" />
                                    <Skeleton className="h-4 w-16 rounded bg-white/10 justify-self-end" />
                                    <Skeleton className="h-4 w-8 rounded bg-white/10 justify-self-end" />
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          <div className="flex flex-col lg:flex-row gap-6 items-stretch animate-pulse">
                              ...
                              <div className="bg-white/2 border border-white/5 rounded-xl p-4 flex flex-col gap-4">
                                ...
                                {[...Array(3)].map((_, i) => (
                                  <div key={i} className="flex flex-col gap-2">
                                    ...
                                    <Skeleton className="h-1.5 w-full rounded bg-white/10" />
                                  </div>
                                ))}
                              </div>
                              <div className="bg-teal-500/5 border border-teal-500/10 rounded-xl p-4 flex gap-3">
                                <Skeleton className="h-4 w-4 rounded-full shrink-0 bg-white/10" />
                            ...
                            <div className="flex-1 flex flex-col gap-4">
                              <div className="flex flex-col sm:flex-row gap-3 justify-between">
                                <Skeleton className="h-8 w-1/3 rounded bg-white/10" />
                                <Skeleton className="h-8 w-1/3 rounded bg-white/10" />
                              </div>
                              <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20 p-3 flex flex-col gap-3">
                                <div className="grid grid-cols-12 p-3 bg-white/5 border-b border-white/5">
                                  <Skeleton className="col-span-1 h-3 w-4 rounded bg-white/10 mx-auto" />
                                  <Skeleton className="col-span-3 h-3 w-1/2 rounded bg-white/10" />
                                  <Skeleton className="col-span-2 h-3 w-8 rounded bg-white/10 mx-auto" />
                                  <Skeleton className="col-span-3 h-3 w-16 rounded bg-white/10 ml-auto" />
                                  <Skeleton className="col-span-3 h-3 w-12 rounded bg-white/10 ml-auto" />
                                </div>
                                {[...Array(4)].map((_, i) => (
                                  <div key={i} className="grid grid-cols-12 p-3 border-b border-white/5 last:border-b-0">
                                    <Skeleton className="col-span-1 h-4 w-4 rounded bg-white/10 mx-auto" />
                                    <Skeleton className="col-span-3 h-4 w-2/3 rounded bg-white/10" />
                                    <Skeleton className="col-span-2 h-4 w-8 rounded bg-white/10 mx-auto" />
                                    <Skeleton className="col-span-3 h-4 w-12 rounded bg-white/10 ml-auto" />
                                    <Skeleton className="col-span-3 h-4 w-10 rounded bg-white/10 ml-auto" />
                                  </div>
                                ))}
                              </div>
                            </div>
              ...
              {/* RQ2: Konsentrasi Genre (Spotlight Cards / City Focus) */}
              {genreSpotlights.length > 0 && (
                <GlassCard className="p-6 flex flex-col gap-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      ...
                      <Text variant="caption" color="secondary" className="mt-1">
                        {rq2Mode === 'genre'
                          ...
                          : "Melihat persentase persebaran genre musik utama yang paling dominan untuk masing-masing kota metropolitan (Data Absolut)."
                        }
                      </Text>
                    </div>
                    ...
                    {/* RQ2 Mode Toggle */}
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-[10px] font-bold text-sky-400 tracking-wider uppercase">Fokus Analisis</span>
                      <div className="flex p-0.5 bg-black/40 rounded-lg border border-white/5">
                        {[
                          { label: "Fokus Genre", value: "genre" },
                          { label: "Fokus Kota", value: "city" }
                        ].map((m) => (
                          <button
                            key={m.value}
                            onClick={() => {
                              if (rq2Mode === m.value) return;
                              setIsRq2Transitioning(true);
                              setRq2Mode(m.value as any);
                              setTimeout(() => setIsRq2Transitioning(false), 300);
                            }}
                            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                              rq2Mode === m.value
                      ...
                      {isRq2Transitioning ? (
                        ...
                        >
                          {[...Array(3)].map((_, i) => (
                            <div key={i} className="pb-3">
                              <GlassCard className="p-4 border-white/5 animate-pulse flex flex-col gap-4">
                                <div className="flex justify-between">
                                  <Skeleton className="h-4 w-1/3 rounded bg-white/10" />
                                  <Skeleton className="h-4 w-12 rounded bg-white/10" />
                                </div>
                        ...
                        >
                          {(showAllGenres ? genreSpotlights : genreSpotlights.slice(0, 5)).map((spotlight) => (
                            <div className="pb-3" key={spotlight.name}>
                              <GlassCard
                          ...
                          {genreSpotlights.length > 5 && (
                            ...
                            >
                              {showAllGenres ? (
                                ...
                                <>
                                  <span>Lihat Lebih Banyak ({genreSpotlights.length - 5} Genre Lainnya)</span>
                              ...
                              <GlassCard className="p-4 border-white/5 hover:border-sky-500/20 transition-all flex flex-col gap-4 relative overflow-hidden bg-gradient-to-r from-sky-500/10 to-sky-600/5 hover:border-sky-500/20">
                                <div className="flex justify-between items-center">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-bold text-white">{city.cityName}</span>
                                  </div>
                                  <span className="text-xs text-white/50 font-mono font-semibold">{city.totalArtists} Musisi</span>
                                </div>
                                  ...
                                  <div className="h-3 w-full bg-white/5 rounded-full overflow-hidden flex border border-white/5">
                                    {city.genres.map((g) => (
                                      <div
                                        ...
                                        title={`${g.name}: ${g.percentage}% (${g.count} Musisi)`}
                                      />
                                    ))}
                                  </div>
                                    ...
                                    {city.genres.slice(0, 5).map((g) => (
                                      <div key={g.name} className="flex items-center gap-1.5 min-w-0">
                                        ...
                                        <span className="text-[10px] text-white/70 font-medium truncate">
                                          {g.name} <span className="text-white/40">({g.percentage}%)</span>
                                        </span>
                                      </div>
                                    ))}
                                    {city.genres.length > 5 && (
                                      <span className="text-[10px] text-white/30 italic">+{city.genres.length - 5} lainnya</span>
                                    )}
                                  </div>
                                </div>
                              </GlassCard>
